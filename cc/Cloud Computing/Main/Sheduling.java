...
public class Sheduling {
    public static double[] fcfs(int noOfTasks, Task[] tasks, boolean debug ) {
        ...
        for (int i = 0; i < noOfTasks; i++) {
            ...
            // responseTime = tasks[i].getBurstTime() - tasks[i].getArrivalTime(); // ArrivalTime = 0
            if(debug == true)
                System.out.println("ProcessID: " + tasks[i].getTaskId() + ", Compilation Time: " + compilationTime + ", Turn Around Time: " + turnAroundTime + ", Waiting Time: " + waitingTime + ", responseTime: " + responseTime);
        }
        if(debug == true){
        System.out.println("--------------------------------------------");
