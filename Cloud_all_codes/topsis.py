...
attributes = np.array(["GRE", "GPA", "College ranking",
                       "Recommendation Rating", "Interview Rating"])
candidates = np.array(
    ["Alfred", "Beverly", "Calvin", "Diane", "Edward", "Fran"])
raw_data = np.array([
    [690, 3.1,  9,  7,  4],
    [590, 3.9,  7,  6, 10],
    [600, 3.6,  8,  8,  7],
...
pd.DataFrame(data=zip(sp, sn, cs), index=candidates,
             columns=["$S^*$", "$S^-$", "$C^*$"])
...
pd.DataFrame(data=zip(cs_order, sp_order, sn_order), index=range(
    1, m + 1), columns=["$C^*$", "$S^*$", "$S^-$"])
